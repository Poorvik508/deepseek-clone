...
const ChartLable = ({ openMenu, setOpenMenu }) => {
  return (
    <div className="flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer">
      <p className="group-hover:max-w-[83%] truncate">Chart Name Here</p>
      <div className="group relative flex items-center justify-center h-6 w-6 aspect-square hover:bg-black/80 rounded-lg">
        <Image
          src={assets.three_dots}
          alt=""
          className={clsx("w-4", openMenu.open ? "" : "hidden")}
        />
        <div
          className={clsx(
            "absolute -right-36 top-6 bg-gray-700 rounded-xl w-max p-2 group-hover:block",
